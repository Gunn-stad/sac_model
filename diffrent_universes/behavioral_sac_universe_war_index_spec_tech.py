...
Clean v6 hybrid SAC portfolio model: Defense/aerospace, index funds, and speculative tech theme test.
...
Usage in Colab:
  ...
  !python behavioral_sac_universe_war_index_spec_tech.py
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "universe_tests", "war_index_spec_tech")
...
TOP20_TICKERS = [
    # Defense/aerospace, index funds, and speculative tech theme test.
    'SPY',
    'QQQ',
    'DIA',
    'VTI',
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "war_index_spec_tech"
    bottom_k: int = 5
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_war_index_spec_tech.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_war_index_spec_tech.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_war_index_spec_tech.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_war_index_spec_tech",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_war_index_spec_tech_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_war_index_spec_tech.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "episode_log_war_index_spec_tech.json")
