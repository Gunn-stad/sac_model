...
How to use:
    ...
    Run in Colab:
        ...
        !python v6_experiments/v6_exp_seed_sweep_2seeds.py
...
def patch_seed(text, seed):
    text = re.sub(r"SEED\s*=\s*\d+", f"SEED = {seed}", text, count=1)
    text = re.sub(r"seed\s*=\s*\d+", f"seed={seed}", text, count=1)
    return text
...
def main():
    ...
    for seed in [0, 1]:
        text = patch_results_name(original, f"v6_seed_sweep_seed{seed}")
        text = patch_seed(text, seed)
        tmp_name = f"_tmp_v6_seed_sweep_seed{seed}.py"
        tmp = Path("/content/sac_model") / tmp_name
        tmp.write_text(text, encoding="utf-8")
        print(f"[run seed {seed}]", tmp)
        code = os.system(f"python {tmp}")
        if code != 0:
