...
Clean v6 hybrid SAC portfolio model: Defensive/stress-rotation assets only.
...
Usage in Colab:
  ...
  !python behavioral_sac_universe_defensive_assets.py
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "universe_tests", "defensive_assets")
...
TOP20_TICKERS = [
    # Defensive/stress-rotation assets only.
    'XLP',
    'XLU',
    'XLV',
    'TLT',
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "defensive_assets"
    bottom_k: int = 5
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_defensive_assets.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_defensive_assets.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_defensive_assets.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_defensive_assets",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_defensive_assets_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_defensive_assets.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "episode_log_defensive_assets.json")
