"""
behavioral_sac_model_v5_1_behavioral.py
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_1_behavioral.py
"""
...
def make_behavioral_finance_features(close: pd.DataFrame, returns: pd.DataFrame, H: pd.DataFrame, tickers: List[str]) -> pd.DataFrame:
    """Human behavioral-finance features: panic, euphoria, overreaction, herding."""
    blocks = []
    market_ret = returns.mean(axis=1)
    market_z = safe_zscore(market_ret, 24)
    for t in tickers:
        r = returns[t].astype(float)
        z = safe_zscore(r, 24).astype(float)
        ...
        f[f"{t}_panic_indicator"] = ((z < -2.0).astype(float) * np.maximum(attention, 0.0) * (1.0 + np.maximum(-sentiment, 0.0)))
        f[f"{t}_euphoria_indicator"] = ((z > 2.0).astype(float) * np.maximum(attention, 0.0) * (1.0 + np.maximum(sentiment, 0.0)))
        f[f"{t}_overreaction_abs"] = z.abs() * (1.0 + np.maximum(attention, 0.0))
        f[f"{t}_loss_aversion_pressure"] = np.maximum(-z, 0.0) * (1.0 + np.maximum(-sentiment, 0.0))
        f[f"{t}_herding_attention"] = np.maximum(attention, 0.0) * (1.0 - disagreement.clip(0, 1))
        f[f"{t}_idiosyncratic_shock"] = rel_z
        blocks.append(f)
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_behavioral.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_1_behavioral.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_behavioral.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_v5_1_behavioral",
...
def main() -> None:
    ...
    B = make_behavioral_finance_features(close.reindex(X_market.index), returns.reindex(X_market.index), H, cfg.tickers)
    H = pd.concat([H, B], axis=1).replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(np.float32)
    meta["n_behavioral_finance_derived_features"] = int(B.shape[1])
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v5_1_behavioral_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v5_1_behavioral.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v5_1_behavioral.json")
