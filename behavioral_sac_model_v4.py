...
Usage in Colab:
  ...
  !python behavioral_sac_model_v3.py
"""
...
class ModelConfig:
    ...
    w_max: float = 0.35
    lambda_risk: float = 0.01     # v4: lower risk penalty so the agent does not hide in cash too much
    lambda_conc: float = 0.0005  # v4: lower concentration penalty
    lambda_exp: float = 0.000    # keep direct exposure penalty off
    lambda_cash: float = -0.0005 # v4: small penalty for sitting in cash
    lambda_bench: float = 0.75   # v4: stronger reward for beating equal-weight benchmark
    min_exposure: float = 0.40   # v4: force at least 40% stock exposure
    max_exposure: float = 1.00   # v4: allow full stock exposure
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v3.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v3.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v3.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v4",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v4_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v4.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v4.json")
