...
def main():
    ...
    text = patch_results_name(text, "v6_higher_exposure")

    text = re.sub(r"MIN_EXPOSURE\s*=\s*[0-9.eE_-]+", "MIN_EXPOSURE = 0.85", text)
    text = re.sub(r"CASH_PENALTY\s*=\s*[0-9.eE_-]+", "CASH_PENALTY = 0.003", text)

    run_patched(text, "_tmp_v6_higher_exposure.py")
