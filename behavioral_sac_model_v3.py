...
class ModelConfig:
    ...
    lambda_conc: float = 0.001
    lambda_exp: float = 0.000
    lambda_cash: float = 0.000
    lambda_bench: float = 0.25
...
class BehavioralContrarianPortfolioEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        exposure = float(sigmoid(action[0]))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v3",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v3_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v3.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v3.json")
