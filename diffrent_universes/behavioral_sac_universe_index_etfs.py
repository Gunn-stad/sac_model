...
Clean v6 hybrid SAC portfolio model: Broad index ETFs only.
...
Usage in Colab:
  ...
  !python behavioral_sac_universe_index_etfs.py
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "universe_tests", "index_etfs_only")
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "index_etfs_only"
    bottom_k: int = 5
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_index_etfs_only.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_index_etfs_only.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_index_etfs_only.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_index_etfs_only",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_index_etfs_only_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_index_etfs_only.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "episode_log_index_etfs_only.json")
