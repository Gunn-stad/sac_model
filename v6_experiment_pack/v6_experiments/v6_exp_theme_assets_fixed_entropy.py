"""
v6_exp_theme_assets_fixed_entropy.py
...
def main():
    candidates = [
        Path("/content/sac_model/behavioral_sac_model_theme_war_index_spec_tech.py"),
        Path("/content/sac_model/diffrent_universes/behavioral_sac_universe_war_index_spec_tech.py"),
    ]
    base = None
    for c in candidates:
        if c.exists():
            ...
            break
    if base is None:
        raise FileNotFoundError("Could not find theme war/index/spec-tech script.")

    print("[base] using", base)
    text = base.read_text(encoding="utf-8")
    text = patch_results_name(text, "v6_theme_assets_fixed_entropy")
    text = re.sub(r'ent_coef\s*=\s*["\']auto["\']', 'ent_coef=0.005', text)

    run_patched(text, "_tmp_v6_theme_assets_fixed_entropy.py")
