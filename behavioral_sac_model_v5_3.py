...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_3.py
"""
...
class ModelConfig:
    ...
    min_exposure: float = 0.75   # v5.3: keep v5.1/v5.2 higher exposure floor
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_3.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_3.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_3.csv"))


# ======================
# Baselines and env
# ======================
def contrarian_weights_dynamic_k_with_regime_filter(close: pd.DataFrame, tickers: List[str], cfg: ModelConfig) -> pd.DataFrame:
    """
    v5.3: dynamic bottom-k plus a regime filter.

    Contrarian mode is active only when the market looks oversold or stressed:
      - recent market return is negative, or
      - volatility is in the upper third.

    Otherwise, the baseline becomes broad equal-weight. This prevents the model from
    forcing contrarian loser-picking in calm upward markets.
    """
    ret24 = close.pct_change(24).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    market_ret1 = close.pct_change().mean(axis=1).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    market_ret24 = (1.0 + market_ret1).rolling(24, min_periods=6).apply(np.prod, raw=True).fillna(1.0) - 1.0
    vol24 = market_ret1.rolling(24, min_periods=6).std().fillna(method="bfill").fillna(0.0)
    vol_low = float(vol24.quantile(0.33))
    vol_high = float(vol24.quantile(0.67))

    rows = []
    regimes = []
    ks = []
    for ts, row in ret24.iterrows():
        v = float(vol24.loc[ts]) if ts in vol24.index else 0.0
        m24 = float(market_ret24.loc[ts]) if ts in market_ret24.index else 0.0
        contrarian_active = (m24 < -0.0025) or (v >= vol_high)
        ...
        if contrarian_active:
            if v >= vol_high:
                ...
                k = cfg.bottom_k
            k = int(max(1, min(k, len(tickers))))
            selected = row.nsmallest(k).index.tolist()
            for t in selected:
                w[tickers.index(t)] = 1.0 / k
            regimes.append("contrarian")
            ks.append(k)
        else:
            # Broad baseline in calm/non-oversold regimes. This gives SAC access to all names
            # through the action mask instead of forcing bottom-k loser selection.
            w[:] = 1.0 / len(tickers)
            regimes.append("broad")
            ks.append(len(tickers))

        w = project_to_caps(w, cfg.w_max)
        rows.append(w)

    out = pd.DataFrame(rows, index=close.index, columns=tickers, dtype=np.float32)
    out.attrs["regime_counts"] = {r: int(regimes.count(r)) for r in sorted(set(regimes))}
    out.attrs["dynamic_k_counts"] = {str(k): int(ks.count(k)) for k in sorted(set(ks))}
    return out
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v5_3",
...
def main() -> None:
    ...
    base_w = contrarian_weights_dynamic_k_with_regime_filter(close.reindex(X.index), cfg.tickers, cfg)
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v5_3_final.zip")
    ...
    bottom = evaluate_strategy("DynamicK_RegimeFiltered_Baseline", r_test, b_test, cfg)
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v5_3.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v5_3.json")
