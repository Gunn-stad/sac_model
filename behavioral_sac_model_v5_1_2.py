"""
behavioral_sac_model_v5_1_2.py

Behavioral / event-aware SAC portfolio model: v5.1.2 fixed-configuration multi-start seed sweep.
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_1_2.py
"""
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_1.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v5_1",
...
# ======================
# v5.1.1 tuning utilities
# ======================

def evaluate_sac_model_named(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig, name: str) -> Dict[str, float]:
    obs, _ = env.reset()
    done = False
    while not done:
        action, _ = model.predict(obs, deterministic=True)
...
def prepare_data(cfg: ModelConfig):
    """Download/load data once and build the feature matrices used by all tuning runs."""
    ...
    meta.update({
        ...
        "tuning_note": "v5.1.1 tunes exposure floor, benchmark reward, learning rate, and seed.",
    })
    save_feature_diagnostics(X_market, H, meta)
    return split_time(X, returns, base_w, cfg)


def run_one_config(run_id: str, cfg: ModelConfig, data_splits) -> Dict[str, Any]:
    """Train one SAC model, select best checkpoint by validation callback, evaluate on validation and test."""
    ...
    X_train, r_train, b_train, X_val, r_val, b_val, X_test, r_test, b_test = data_splits
    run_dir = os.path.join(RESULTS_DIR, "v5_1_2_seed_sweep", run_id)
    os.makedirs(run_dir, exist_ok=True)

    train_env = Monitor(BehavioralContrarianPortfolioEnv(X_train, r_train, b_train, cfg))
    val_env_for_callback = Monitor(BehavioralContrarianPortfolioEnv(X_val, r_val, b_val, cfg))

    eval_callback = EvalCallback(
        val_env_for_callback,
    ...
    model_eval = SAC.load(best_path) if os.path.exists(best_path) else model

    val_env = BehavioralContrarianPortfolioEnv(X_val, r_val, b_val, cfg)
    test_env = BehavioralContrarianPortfolioEnv(X_test, r_test, b_test, cfg)
    val_metrics = evaluate_sac_model_named(model_eval, val_env, cfg, "VAL_SAC_v5_1_2")
    test_metrics = evaluate_sac_model_named(model_eval, test_env, cfg, "TEST_SAC_v5_1_2")

    # A simple validation selection score: prioritise Sharpe, then return, penalise drawdown mildly.
    val_score = (
        float(np.nan_to_num(val_metrics["sharpe"], nan=-999.0))
        + 5.0 * float(val_metrics["total_return"])
    ...
    row = {
        ...
        "val_cash": val_metrics["avg_cash"],
        "test_return": test_metrics["total_return"],
        "test_sharpe": test_metrics["sharpe"],
        "test_drawdown": test_metrics["max_drawdown"],
        "test_cash": test_metrics["avg_cash"],
...
def main() -> None:
    """
    v5.1.1 tuning script.

    Default grid is deliberately small enough to run in Colab:
      exposure floors: 0.75, 0.80
      benchmark rewards: 1.25, 1.75
      learning rate: 1e-4
      seeds: 0, 1
    Total: 8 runs.

    You can override total training length with:
      import os; os.environ["SAC_TUNE_TIMESTEPS"] = "120000"
    """
    base_cfg = ModelConfig(tickers=TOP20_TICKERS)
    tune_timesteps = int(os.environ.get("SAC_TUNE_TIMESTEPS", "120000"))

    data_splits = prepare_data(base_cfg)
    X_train, r_train, b_train, X_val, r_val, b_val, X_test, r_test, b_test = data_splits
    print("Rows (train/val/test):", len(X_train), len(X_val), len(X_test))

    # Baseline references on the same test split.
    baseline_eq = evaluate_strategy("Baseline_EQ", r_test, None, base_cfg)
    baseline_bottom = evaluate_strategy(f"Bottom{base_cfg.bottom_k}_ret_24h", r_test, b_test, base_cfg)
    print("\n=== Baseline references on test ===")
    print(baseline_eq)
    print(baseline_bottom)

    exposure_floors = [0.75]
    lambda_benches = [1.25]
    learning_rates = [1e-4]
    seeds = list(range(10))

    all_rows = []
    for min_exp in exposure_floors:
        for lb in lambda_benches:
            for lr in learning_rates:
                for seed in seeds:
                    run_id = f"exp{str(min_exp).replace('.', 'p')}_bench{str(lb).replace('.', 'p')}_lr{lr:.0e}_seed{seed}"
                    cfg = replace(
                        base_cfg,
                    ...
                    row = run_one_config(run_id, cfg, data_splits)
                    all_rows.append(row)

                    # Save after every run so progress is not lost if Colab disconnects.
                    out_dir = os.path.join(RESULTS_DIR, "v5_1_2_seed_sweep")
                    os.makedirs(out_dir, exist_ok=True)
                    pd.DataFrame(all_rows).sort_values("val_score", ascending=False).to_csv(
                        os.path.join(out_dir, "seed_sweep_summary_v5_1_2_partial.csv"), index=False
                    )

    out_dir = os.path.join(RESULTS_DIR, "v5_1_2_seed_sweep")
    os.makedirs(out_dir, exist_ok=True)
    summary = pd.DataFrame(all_rows).sort_values("val_score", ascending=False)
    summary_path = os.path.join(out_dir, "seed_sweep_summary_v5_1_2.csv")
    summary.to_csv(summary_path, index=False)

    baseline_path = os.path.join(out_dir, "baseline_reference_v5_1_2.json")
    with open(baseline_path, "w", encoding="utf-8") as f:
        json.dump({"Baseline_EQ": baseline_eq, "Bottom5": baseline_bottom}, f, indent=2)

    print("\n=== v5.1.2 seed sweep complete ===")
    print("[saved] seed sweep summary ->", summary_path)
    print("[saved] baseline reference ->", baseline_path)
    print("\nTop seeds by validation score:")
    print(summary.head(10).to_string(index=False))

    if len(summary):
        best = summary.iloc[0].to_dict()
        print("\nBest validation-selected seed/configuration:")
