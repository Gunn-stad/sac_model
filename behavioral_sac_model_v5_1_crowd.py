"""
behavioral_sac_model_v5_1_crowd.py
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_1_crowd.py
"""
...
def make_crowd_signal_features(H: pd.DataFrame, tickers: List[str]) -> pd.DataFrame:
    """Derived wisdom-of-the-crowd features from loaded behavioral/crowd data."""
    blocks = []
    for t in tickers:
        sentiment = H.get(f"{t}_sentiment_score", pd.Series(0.0, index=H.index)).astype(float)
        attention = H.get(f"{t}_attention_shock", H.get(f"{t}_google_trend_z", pd.Series(0.0, index=H.index))).astype(float)
        disagreement = H.get(f"{t}_crowd_disagreement", pd.Series(0.0, index=H.index)).astype(float)
        news_z = H.get(f"{t}_news_count_z", pd.Series(0.0, index=H.index)).astype(float)
        neg = H.get(f"{t}_news_negative_share", pd.Series(0.0, index=H.index)).astype(float)
        f = pd.DataFrame(index=H.index)
        f[f"{t}_crowd_hype"] = np.maximum(sentiment, 0.0) * np.maximum(attention, 0.0)
        f[f"{t}_crowd_panic"] = np.maximum(-sentiment, 0.0) * np.maximum(attention, 0.0)
        f[f"{t}_crowd_disagreement_x_attention"] = disagreement * np.maximum(attention, 0.0)
        f[f"{t}_crowd_news_attention"] = news_z + attention
        f[f"{t}_crowd_negative_news_pressure"] = neg * np.maximum(news_z, 0.0)
        blocks.append(f)
    sentiment_cols = [c for c in H.columns if c.endswith("_sentiment_score")]
    attention_cols = [c for c in H.columns if c.endswith("_attention_shock") or c.endswith("_google_trend_z")]
    global_df = pd.DataFrame(index=H.index)
    if sentiment_cols:
        global_df["crowd_market_sentiment_mean"] = H[sentiment_cols].mean(axis=1)
        global_df["crowd_market_sentiment_dispersion"] = H[sentiment_cols].std(axis=1).fillna(0.0)
    else:
        global_df["crowd_market_sentiment_mean"] = 0.0
        global_df["crowd_market_sentiment_dispersion"] = 0.0
    if attention_cols:
        global_df["crowd_market_attention_mean"] = H[attention_cols].mean(axis=1)
        global_df["crowd_market_attention_max"] = H[attention_cols].max(axis=1)
    else:
        global_df["crowd_market_attention_mean"] = 0.0
        global_df["crowd_market_attention_max"] = 0.0
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_crowd.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_1_crowd.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_crowd.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_v5_1_crowd",
...
def main() -> None:
    ...
    C = make_crowd_signal_features(H, cfg.tickers)
    H = pd.concat([H, C], axis=1).replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(np.float32)
    meta["n_crowd_derived_features"] = int(C.shape[1])
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v5_1_crowd_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v5_1_crowd.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v5_1_crowd.json")
