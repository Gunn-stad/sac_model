...
Usage in Colab:
  ...
  !python behavioral_sac_model_v6.py
"""
...
class ModelConfig:
    ...
    lambda_cash: float = -0.0020 # v5: stronger penalty for sitting in cash
    lambda_bench: float = 1.25   # v5: stronger reward for beating equal-weight benchmark
    min_exposure: float = 0.65   # v5: force at least 65% stock exposure
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v6.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v6.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v6.csv"))
...
def contrarian_weights_from_features(close: pd.DataFrame, tickers: List[str], bottom_k: int, w_max: float) -> pd.DataFrame:
    """Original pure contrarian baseline: choose bottom-k by 24h return."""
...
def behavioral_contrarian_weights_from_features(
    close: pd.DataFrame,
    ...
    """
    v6 behaviorally ranked contrarian baseline.

    Pure contrarian chooses the bottom-k losers by recent return. v6 modifies
    that selection by asking: is the loser possibly oversold, or is the loser
    surrounded by genuinely bad news?

    Lower score is selected. The score is approximately:

        score_i = ret24_i
                  - 0.020 * sentiment_i
                  - 0.010 * attention_i
                  - 0.010 * overreaction_i
                  + 0.020 * negative_news_i
                  + 0.010 * event_shock_i
                  + 0.010 * panic_i

    Interpretation:
      - Negative 24h return still makes a stock attractive for contrarian selection.
      - Positive sentiment / attention / overreaction makes it more likely to be an oversold rebound candidate.
      - Negative news, event shocks, and panic make it less attractive because it may be a falling knife.
    """
    ret24 = close.pct_change(24).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    H = behavioral.reindex(close.index).ffill().fillna(0.0)

    rows = []
    selected_log = []
    for ts, row in ret24.iterrows():
        scores = row.copy().astype(float)
        for t in tickers:
            sentiment = float(H.loc[ts].get(f"{t}_sentiment_score", 0.0))
            attention = float(H.loc[ts].get(f"{t}_attention_shock", 0.0))
            trend = float(H.loc[ts].get(f"{t}_google_trend_z", 0.0))
            overreaction = float(H.loc[ts].get(f"{t}_proxy_overreaction", 0.0))
            negative = float(H.loc[ts].get(f"{t}_news_negative_share", 0.0))
            event = float(H.loc[ts].get(f"{t}_event_shock", 0.0))
            panic = float(H.loc[ts].get(f"{t}_proxy_panic", 0.0))
            ...
            scores.loc[t] = float(scores.loc[t]) + behavioral_adjustment

        selected = scores.nsmallest(bottom_k).index.tolist()
        selected_log.append({"datetime": str(ts), "selected": selected})
        w = np.zeros(len(tickers), dtype=np.float32)
        for t in selected:
            w[tickers.index(t)] = 1.0 / bottom_k
        w = project_to_caps(w, w_max)
    ...
    out = pd.DataFrame(rows, index=close.index, columns=tickers, dtype=np.float32)
    try:
        with open(os.path.join(RESULTS_DIR, "behavioral_contrarian_selected_v6.json"), "w", encoding="utf-8") as f:
            json.dump(selected_log[-500:], f, indent=2)
    except Exception:
        pass
    return out
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v6",
...
def main() -> None:
    ...
    # v6: behaviorally ranked contrarian basket.
    # Keep the pure return-only baseline separately for comparison.
    pure_base_w = contrarian_weights_from_features(close.reindex(X.index), cfg.tickers, cfg.bottom_k, cfg.w_max)
    base_w = behavioral_contrarian_weights_from_features(close.reindex(X.index), H, cfg.tickers, cfg.bottom_k, cfg.w_max)
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v6_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v6.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v6.json")
