"""
behavioral_sac_model_v5_1_1.py
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_1.py
"""
...
def run_one_config(run_id: str, cfg: ModelConfig, data_splits) -> Dict[str, Any]:
    ...
    run_dir = os.path.join(RESULTS_DIR, "v5_1_1_tuning", run_id)
    ...
    val_metrics = evaluate_sac_model_named(model_eval, val_env, cfg, "VAL_SAC_v5_1_1")
    test_metrics = evaluate_sac_model_named(model_eval, test_env, cfg, "TEST_SAC_v5_1_1")
...
def main() -> None:
    ...
    tune_timesteps = int(os.environ.get("SAC_TUNE_TIMESTEPS", "80000"))
    ...
    exposure_floors = [0.75, 0.80]
    lambda_benches = [1.25, 1.75]
    learning_rates = [1e-4]
    seeds = [0, 1]
        ...
        for lb in lambda_benches:
            for lr in learning_rates:
                for seed in seeds:
                    ...
                    out_dir = os.path.join(RESULTS_DIR, "v5_1_1_tuning")
                    os.makedirs(out_dir, exist_ok=True)
                    pd.DataFrame(all_rows).sort_values("val_score", ascending=False).to_csv(
                        os.path.join(out_dir, "tuning_summary_v5_1_1_partial.csv"), index=False
                    )

    out_dir = os.path.join(RESULTS_DIR, "v5_1_1_tuning")
    os.makedirs(out_dir, exist_ok=True)
    summary = pd.DataFrame(all_rows).sort_values("val_score", ascending=False)
    summary_path = os.path.join(out_dir, "tuning_summary_v5_1_1.csv")
    summary.to_csv(summary_path, index=False)

    baseline_path = os.path.join(out_dir, "baseline_reference_v5_1_1.json")
    with open(baseline_path, "w", encoding="utf-8") as f:
        json.dump({"Baseline_EQ": baseline_eq, "Bottom5": baseline_bottom}, f, indent=2)

    print("\n=== v5.1.1 tuning complete ===")
    print("[saved] tuning summary ->", summary_path)
    print("[saved] baseline reference ->", baseline_path)
    print("\nTop configurations by validation score:")
