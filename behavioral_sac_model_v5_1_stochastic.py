"""
behavioral_sac_model_v5_1_stochastic.py
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_1_stochastic.py
"""
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_stochastic.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_1_stochastic.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_1_stochastic.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_v5_1_stochastic",
...
def main() -> None:
    ...
    Q = make_stochastic_process_features(close.reindex(X_market.index), returns.reindex(X_market.index), cfg.tickers)
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v5_1_stochastic_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v5_1_stochastic.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v5_1_stochastic.json")
