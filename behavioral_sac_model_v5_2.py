...
Usage in Colab:
  ...
  !python behavioral_sac_model_v5_2.py
"""
...
class ModelConfig:
    ...
    min_exposure: float = 0.75   # v5.2: keep v5.1 higher exposure floor
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_2.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v5_2.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v5_2.csv"))


# ======================
# Baselines and env
# ======================
def contrarian_weights_dynamic_bottom_k(close: pd.DataFrame, tickers: List[str], cfg: ModelConfig) -> pd.DataFrame:
    """
    v5.2: choose the contrarian basket size dynamically from market regime.

    Intuition:
      - High volatility / panic: use fewer names (bottom 3) to avoid buying too many falling knives.
      - Normal regime: use bottom 5.
      - Quiet/stable regime: use bottom 7 to diversify the contrarian basket.

    The ranking itself is still pure 24h return, so this isolates the effect of dynamic k.
    """
    ret24 = close.pct_change(24).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    market_ret = close.pct_change().mean(axis=1).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    vol24 = market_ret.rolling(24, min_periods=6).std().fillna(method="bfill").fillna(0.0)
    ...
    out.attrs["dynamic_k_mean"] = float(np.mean(ks)) if ks else float(cfg.bottom_k)
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_Behavioral_SAC_v5_2",
...
def main() -> None:
    ...
    base_w = contrarian_weights_dynamic_bottom_k(close.reindex(X.index), cfg.tickers, cfg)
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_top20_v5_2_final.zip")
    ...
    bottom = evaluate_strategy("DynamicBottomK_ret_24h", r_test, b_test, cfg)
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v5_2.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v5_2.json")
