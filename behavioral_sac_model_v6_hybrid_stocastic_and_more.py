"""
behavioral_sac_model_v6_hybrid.py

Hybrid SAC portfolio model: stochastic-process features + expanded tradable universe.

This variant tests whether adding broad index ETFs, defensive ETFs, defense/aerospace
ETFs, and speculative/high-growth technology ETFs gives the agent better ways to
rotate risk, while stochastic-process features help distinguish normal returns
from volatility shocks, jumps, drawdowns, and market stress.
...
Usage in Colab:
  ...
  !python behavioral_sac_model_v6_hybrid.py
"""
...
TOP20_TICKERS = [
    ...
    # Defense / aerospace ETFs. These are used to test the “war/defense” hypothesis.
    "ITA", "PPA", "XAR",
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "top20_hybrid_assets_stochastic"
    bottom_k: int = 5
...
def main() -> None:
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_v6_hybrid.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_v6_hybrid.json")
