"""
behavioral_sac_model_theme_war_index_spec_tech.py

Separate theme-universe experiment for testing whether broad index funds,
defense/aerospace ETFs, and speculative/high-growth technology ETFs improve
growth in the SAC portfolio framework.

This is NOT the main v6 hybrid model. It is a separate hypothesis test.
The theme universe includes index funds, defense/aerospace, speculative tech,
and defensive/stress assets so the agent can rotate risk rather than only hold cash.
...
Usage in Colab:
  ...
  !python behavioral_sac_model_theme_war_index_spec_tech.py
"""
...
TOP20_TICKERS = [
    # Broad index funds / market beta
    "SPY", "QQQ", "DIA", "VTI", "IWM",

    # Defense / aerospace ETFs: war/defense hypothesis
    "ITA", "PPA", "XAR",

    # Speculative / high-growth technology themes
    "SMH", "SOXX", "ARKK", "BOTZ", "ICLN",

    # Defensive and market-stress rotation assets
    "XLP", "XLU", "XLV", "TLT", "IEF", "GLD", "SHY",
]
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "theme_war_index_spec_tech"
    bottom_k: int = 4
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v6_hybrid.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_v6_hybrid.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_v6_hybrid.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_v6_hybrid",
...
def main() -> None:
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_v6_hybrid_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_theme_war_index_spec_tech.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "top20_episode_log_theme_war_index_spec_tech.json")
