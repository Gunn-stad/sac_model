...
Clean v6 hybrid SAC portfolio model: Mixed universe: large-cap stocks, index/sector ETFs, bonds, gold, and defensive assets.

This variant combines the strongest directions found so far:
  - v5.1 base reward/exposure structure
  - stochastic-process features
  - ETF universe for broad market, sectors, and defensive rotation
  - contrarian baseline
  - low turnover / smoothing
  - minimum exposure
  - volatility regime classifier, drawdown regime, and market stress index

This file does NOT include the separate war/index/speculative-tech theme test.
That is kept in behavioral_sac_model_theme_war_index_spec_tech.py so experiments remain comparable.
...
Usage in Colab:
  ...
  !python behavioral_sac_universe_mixed_assets.py
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "universe_tests", "mixed_stocks_etfs_defensive")
...
TOP20_TICKERS = [
    # Mixed universe: large-cap stocks, index/sector ETFs, bonds, gold, and defensive assets.
    'AAPL',
    'MSFT',
    'NVDA',
    'AMZN',
...
class ModelConfig:
    ...
    period: str = "730d"
    interval: str = "1h"
    universe_name: str = "mixed_stocks_etfs_defensive"
    bottom_k: int = 5
...
def make_stochastic_process_features(close: pd.DataFrame, returns: pd.DataFrame, tickers: List[str]) -> pd.DataFrame:
    """
    Stochastic-calculus-inspired features for the state vector.
    These approximate the local drift/volatility/shock terms in a GBM-like model:
    ...
    for t in tickers:
        ...
        p = close[t].astype(float)
        f = pd.DataFrame(index=close.index)
        mu_24 = r.rolling(24, min_periods=8).mean().fillna(0.0)
        sig_24 = r.rolling(24, min_periods=8).std(ddof=0).replace(0, np.nan).fillna(1e-8)
        mu_72 = r.rolling(72, min_periods=16).mean().fillna(0.0)
        sig_72 = r.rolling(72, min_periods=16).std(ddof=0).replace(0, np.nan).fillna(1e-8)
        z_24 = ((r - mu_24) / sig_24).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        z_72 = ((r - mu_72) / sig_72).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        f[f"{t}_drift_mu_24"] = mu_24
        f[f"{t}_drift_mu_72"] = mu_72
        f[f"{t}_diffusion_sigma_24"] = sig_24
        f[f"{t}_diffusion_sigma_72"] = sig_72
        f[f"{t}_brownian_shock_z24"] = z_24
        f[f"{t}_brownian_shock_z72"] = z_72
        f[f"{t}_realized_var_24"] = (r ** 2).rolling(24, min_periods=8).sum().fillna(0.0)
        f[f"{t}_realized_var_72"] = (r ** 2).rolling(72, min_periods=16).sum().fillna(0.0)
        f[f"{t}_jump_down_25"] = (z_24 < -2.5).astype(float)
        f[f"{t}_jump_up_25"] = (z_24 > 2.5).astype(float)
        f[f"{t}_jump_abs_25"] = (z_24.abs() > 2.5).astype(float)
        roll_max = p.rolling(72, min_periods=16).max()
        drawdown = (p / roll_max - 1.0).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        f[f"{t}_drawdown_72"] = drawdown
        cov = r.rolling(72, min_periods=16).cov(market_ret)
        var = market_ret.rolling(72, min_periods=16).var(ddof=0).replace(0, np.nan)
        beta_72 = (cov / var).replace([np.inf, -np.inf], np.nan).fillna(1.0)
        f[f"{t}_stoch_beta_72"] = beta_72
        blocks.append(f)
...
def save_feature_diagnostics(X: pd.DataFrame, H: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    diag.to_csv(os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_mixed_stocks_etfs_defensive.csv"), index=False)
    with open(os.path.join(RESULTS_DIR, "feature_metadata_mixed_stocks_etfs_defensive.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("[saved] diagnostics ->", os.path.join(RESULTS_DIR, "behavioral_feature_diagnostics_mixed_stocks_etfs_defensive.csv"))
...
def evaluate_sac_model(model, env: BehavioralContrarianPortfolioEnv, cfg: ModelConfig) -> Dict[str, float]:
    ...
    return {
        "name": "Hybrid_Contrarian_SAC_mixed_stocks_etfs_defensive",
...
def main() -> None:
    ...
    # v6 hybrid addition: stochastic-process features
    Q = make_stochastic_process_features(
        close.reindex(X_market.index),
        returns.reindex(X_market.index),
        cfg.tickers,
    )
    X_market = pd.concat([X_market, Q], axis=1).replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(np.float32)
    ...
    final_model_path = os.path.join(MODELS_DIR, "behavioral_sac_mixed_stocks_etfs_defensive_final.zip")
    ...
    for row in results:
        print(row)
    summary_path = os.path.join(RESULTS_DIR, "summary_results_mixed_stocks_etfs_defensive.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print("[saved] summary ->", summary_path)

    print("\n=== Exporting one human-readable replay ===")
    replay_path = os.path.join(RESULTS_DIR, "episode_log_mixed_stocks_etfs_defensive.json")
